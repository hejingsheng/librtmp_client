...
gboolean rtcp_parse_lost_info(char *packet, int len, uint32_t *lost, int *fraction);
 ...
 * @returns The message data length in bytes, if successful, -1 on errors */
int rtcp_transport_wide_cc_feedback(char *packet, size_t len, guint32 ssrc, guint32 media, guint8 feedback_packet_count, GQueue *transport_wide_cc_stats);

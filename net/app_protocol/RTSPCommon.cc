...
bool parseRTSPURL(const char *url, unsigned &address, unsigned short &port, std::string &urlSuffix)
{
	do {
		...
		if (nextChar == ':') {
			...
			if (sscanf(++from, "%d", &portNumInt) != 1) {
				ELOG("No port number follows\n");
				break;
			}
			if (portNumInt < 1 || portNumInt > 65535) {
				ELOG("Bad port number\n");
				break;
			}
			port = portNumInt;
...
bool parseRTSPURLUsernamePassword(char const* url, std::string &username, std::string &password)
{
	do {
		...
		if (colonIndex > 0) {
			password = std::string(url + colonIndex + 1, atIndex - colonIndex - 1);
			username = std::string(url + usernameIndex, colonIndex - usernameIndex);
		} else {
			password = "";
			username = std::string(url + usernameIndex, atIndex - usernameIndex);
		}

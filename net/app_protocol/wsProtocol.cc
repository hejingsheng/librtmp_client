...
int WebSocketProtocolBase::decodeData(const char *data, int len, std::string &dest, bool &finish, WsOpCode &opcode, int &hlen)
{
	WsHeader wsHeader;
	int headerLen;
	const char *realData;
	...
	{
		dest.append(realData, (len - headerLen));
		//memcpy(dest, realData, wsHeader.len);
	}
	finish = wsHeader.eof;
	opcode = wsHeader.opcode;
	hlen = headerLen;
	return (int)wsHeader.len;
}
...
{
	DLOG("delete ws protocol server\n");
}
...
int WebSocketProtocolClient::doHandShake(std::string &handshakedata)
{
	...
	os << "Sec-WebSocket-Protocol: " << protocol << CRLF;
...
int WebSocketProtocolClient::doResponse(std::string &responsedata)
{
	if (responsedata.find("\r\n\r\n") != std::string::npos)
	{
		...
		if (strcasecmp(upgrade.c_str(), "websocket") || code != "101" || accept.empty())
		{
			return -1;
		}
		bIsConnect = true;
